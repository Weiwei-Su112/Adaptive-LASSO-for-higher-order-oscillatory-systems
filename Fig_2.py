...
def create_binary_lst(threshold, lst1, lst2, lst3, lst4, lst5, lst6, lst7, lst8):
    sub1 = np.copy(lst1)
    sub2 = np.copy(lst2)
    sub3 = np.copy(lst3)
    sub4 = np.copy(lst4)
    ...
    sub1[sub1 != 0] = 1
    sub2[sub2 < threshold] = 0
    sub2[sub2 >= threshold] = 1
    sub3[sub3 < threshold] = 0
    sub3[sub3 >= threshold] = 1
    sub7[sub7 < threshold] = 0
    sub7[sub7 >= threshold] = 1
    sub4[sub4 != 0] = 1
    sub5[sub5 < threshold] = 0
    sub5[sub5 >= threshold] = 1
    sub6[sub6 < threshold] = 0
    sub6[sub6 >= threshold] = 1
    sub8[sub8 < threshold] = 0
    sub8[sub8 >= threshold] = 1

    return sub1, sub2, sub3, sub4, sub5, sub6, sub7, sub8


def mcc_score(lst1, lst2, v1: float or int, v2: float or int):
    a = sum((lst1 == lst2) & (lst1 == v1))
    b = sum((lst1 != lst2) & (lst1 == v2))
    c = sum((lst1 != lst2) & (lst1 == v1))
    d = sum((lst1 == lst2) & (lst1 == v2))
...
new_model_2 = GeneralInteraction(coupling2=cou2, coupling3=0, dt=dt, T=T, natfreqs=natfreq, with_noise=True,
                                 noise_sth=0.2, normalize=True, conn2=p2, all_connected=False,
...
new_model_3 = GeneralInteraction(coupling2=0, coupling3=cou3, dt=dt, T=T, natfreqs=natfreq, with_noise=True,
                                 noise_sth=0.2, normalize=True, conn3=p3, all_connected=False, init_phase=init_phase,
...
test_coup_mix3 = test_conn_mix3 * cou3 / (p3 * 12 * 12)
...
for ptr in range(0, 3):
    ...
    header0 = ["i", "Real", "Estimate - LASSO", "Estimate - OLS", "Estimate - Ada. LASSO"]
    header2 = ["i", "j", "Real", "Estimate - LASSO", "Estimate - OLS", "Estimate - Ada. LASSO"]
    header3 = ["i", "j", "k", "Real", "Estimate - LASSO", "Estimate - OLS", "Estimate - Ada. LASSO"]
