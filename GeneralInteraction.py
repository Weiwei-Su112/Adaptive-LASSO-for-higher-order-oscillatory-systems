...
def reduce_conn_2(conn_2, cut_per=0.5, reduce_seed=None):
    ...
    if not np.isin(conn_2copy, np.array([0, 1])).all():
...
def reduce_conn_3(conn_3, cut_per=0.5, reduce_seed=None):
    ...
    if not np.isin(conn_3copy, np.array([0, 1])).all():
...
class GeneralInteraction:
    def __init__(self, dt=0.01, T=10, n_nodes=None, natfreqs=None, with_noise=True, noise_sth=1, random_sd=False,
                 pre_sd=None, conn: Optional[int or float] = 0, init_phase=None, coupling2=1, coupling3=1,
                 random_coup2=False, random_coup3=False, normal_dis_coup=False,
                 ...
                 for_fdr_empty=False, pre_coup2_sub=None, alpha_phase=None, pre_conn4=None, pre_coup4=None, conn4=None,
                 pre_conn3_sub=None, pre_coup3_sub=None, conn3_sub=None, old_legacy=False):
        """
        coupling: float
        ...
        if alpha_phase is not None and type2 != 4:
            raise ValueError("alpha_phase can only be specified when applying Kuramoto-Sakaguchi (type2 == 4)")
        elif alpha_phase is None and type2 == 4:
            raise ValueError("alpha_phase can only be specified when applying Kuramoto-Sakaguchi (type2 == 4)")
        ...
        if pre_conn2 is None and pre_conn3 is None and pre_coup2 is None and pre_coup3 is None:
            assert conn != 0 or conn2 != 0 or conn3 != 0, "Need to specify the conn params. "

        self.conn2 = conn2
        self.conn3 = conn3
        if self.type2 == 5 and self.type3 == 5:
            self.conn4 = conn4
        if self.type2 == 6 and self.type3 == 6:
            self.conn3_sub = conn3_sub
        ...
        self.coup_seed = None
        if pre_coup2 is None or pre_coup3 is None:
            if coup_seed is None and (random_coup2 or random_coup3):
                self.coup_seed = np.random.randint(low=0, high=1e9)
            elif coup_seed is not None:
                self.coup_seed = coup_seed
                if self.coupling_coef2 != 0:
                    random_coup2 = True
                if self.coupling_coef3 != 0:
                    random_coup3 = True
        ...
        if init_phase is not None:
            ...
            if self.type2 == 3 and self.type3 == 3:
                self.init_phase = max(self.k_ve) * rng.random(size=self.n_nodes) + 10
            else:
        ...
        if conn2 == 0 and conn != 0 and conn is not None:
            conn2 = conn
            # print("conn2 changed.")
        ...
        if conn3 == 0 and conn != 0 and conn is not None:
            conn3 = conn
            # print("conn3 changed.")
        ...
        if np.sum(self.conn_mat2) == 0 and not for_fdr_empty:
            i_lst = random.sample(range(0, self.n_nodes), 2)
            self.conn_mat2[i_lst[0]][i_lst[1]] = 1
        if np.sum(self.conn_mat3) == 0 and not for_fdr_empty and self.n_nodes > 2:
        ...
        if pre_coup2 is None:
            ...
            if random_coup2 is True:
                ...
                if self.type2 == 4:
                    coup_sub_mat = np.zeros(shape=(self.n_nodes, self.n_nodes))
                coupling = self.coupling_coef2 / effective_conn2  # normalization
                for i in range(self.n_nodes):
                    for j in range(self.n_nodes):
                        if i != j:
                            if normal_dis_coup:
                                coup_mat[j][i] = coupling * coup2_rng.normal(loc=0, scale=0.5)
                            else:
                                coup_mat[j][i] = coupling * coup2_rng.lognormal(mean=0.01, sigma=0.5)
                            if self.type2 == 4:
                                coup_sub_mat[j][i] = coup_mat[j][i] * np.sin(alpha_phase)
                                coup_mat[j][i] = coup_mat[j][i] * np.cos(alpha_phase)
            else:
                coup_mat = np.zeros(shape=(self.n_nodes, self.n_nodes))
                coupling = self.coupling_coef2 / effective_conn2  # normalization
                for i in range(self.n_nodes):
            ...
            if self.type2 == 4:
                self.coupling2_sub = self.conn_mat2 * coup_sub_mat
        elif pre_coup2 is not None and pre_conn2 is not None:
            self.coupling2 = self.conn_mat2 * pre_coup2 / effective_conn2
            if self.type2 == 4:
                self.coupling2_sub = self.coupling2 * np.sin(alpha_phase)
                self.coupling2 = self.coupling2 * np.cos(alpha_phase)
        else:
        ...
        if pre_coup3 is None:
            ...
            if random_coup3 is True:
                ...
                coupling = self.coupling_coef3 / effective_conn3  # normalization
                for i in range(self.n_nodes):
                    for k in range(self.n_nodes):
                        for j in range(k):
                            if i != j and i != k and j != k:
                                if normal_dis_coup:
                                    coup_mat[i][j][k] = coupling * coup3_rng.normal(loc=0, scale=0.5)
                                else:
            ...
            self.coupling3 = self.conn_mat3 * pre_coup3 / effective_conn3
        else:
            raise ValueError("Preset coupling3 matrix needs to be paired with preset connectivity matrix. ")

        if self.type2 == 5:
            assert pre_conn4 is not None and pre_coup4 is not None and conn4 is not None, \
                "4-body interaction needs to have pre-defined conn and coup matrix, with the connectivity"
            if normalize and conn4 != 0:
                effective_conn4 = conn4 * self.n_nodes ** 3
            else:
                effective_conn4 = 1
            self.conn_mat4 = pre_conn4
            self.coupling4 = self.conn_mat4 * pre_coup4 / effective_conn4

        if self.type2 == 6 and self.type3 == 6:
            # TODO: now only preset
            assert pre_conn3_sub is not None and pre_coup3_sub is not None and conn3_sub is not None, \
                "4-body interaction needs to have pre-defined conn and coup matrix, with the connectivity"
            if normalize and conn3_sub != 0:
                effective_conn3_sub = conn3_sub * self.n_nodes ** 2
            else:
                effective_conn3_sub = 1
            self.conn_mat3_sub = pre_conn3_sub
            self.coupling3_sub = self.conn_mat3_sub * pre_coup3_sub / effective_conn3_sub

    def get_connectivity(self):
        test2 = np.copy(self.coupling2)
        test3 = np.copy(self.coupling3)

        # TODO: use self.conn_mat directly
        test2[test2 != 0] = 1
        test3[test3 != 0] = 1
    ...
    def load_seed(self, csv_file):
        assert csv_file[-8:] == "seed.csv", "Check seed file name. "
    ...
    def derivative(self, phases_vec, t, coupling2, coupling3):
        ...
        if self.type2 == 3 or self.type3 == 3:
        ...
        elif self.type2 == 4 or self.type3 == 4:
            phases_i, phases_j = np.meshgrid(phases_vec, phases_vec)
            interactions2 = coupling2[0] * np.sin(phases_j - phases_i) + coupling2[1] * np.cos(
                phases_j - phases_i)  # Aji * sin(j-i)
            dxdt = self.natfreqs + interactions2.sum(axis=0)

        elif self.type2 == 5:
            phases_jjj, phases_iii, phases_kkk, phases_lll = np.meshgrid(phases_vec, phases_vec, phases_vec, phases_vec)
            something = np.sin(phases_jjj + phases_kkk - phases_lll - phases_iii) \
                        + np.sin(phases_jjj + phases_lll - phases_kkk - phases_iii) \
                        + np.sin(phases_kkk + phases_lll - phases_jjj - phases_iii)
            interactions4 = self.coupling4 * something

            phases_jj, phases_ii, phases_kk = np.meshgrid(phases_vec, phases_vec, phases_vec)
            something = np.sin(2 * phases_jj - phases_kk - phases_ii) + np.sin(2 * phases_kk - phases_jj - phases_ii)
            ...
            dxdt = self.natfreqs + interactions2.sum(axis=0) + interactions3.sum(axis=1).sum(
                axis=1) + interactions4.sum(axis=1).sum(axis=1).sum(axis=1)  # sum over incoming interactions
        elif self.type2 == 6 and self.type3 == 6:
            phases_jj, phases_ii, phases_kk = np.meshgrid(phases_vec, phases_vec, phases_vec)
            something = np.sin(2 * phases_jj - phases_kk - phases_ii) + np.sin(2 * phases_kk - phases_jj - phases_ii)
            interactions3 = coupling3 * something  # Aijk * sin(j+k-2i)
            ...
            interactions3_sub = self.coupling3_sub * something  # Aijk * sin(j+k-2i)

            phases_i, phases_j = np.meshgrid(phases_vec, phases_vec)
            interactions2 = coupling2 * np.sin(phases_j - phases_i)  # Aji * sin(j-i)
            dxdt = self.natfreqs + interactions2.sum(axis=0) + interactions3.sum(axis=1).sum(
                axis=1) + interactions3_sub.sum(axis=1).sum(axis=1)  # sum over incoming interactions
            ...
            dxdt = self.natfreqs + interactions2.sum(axis=0) + interactions3.sum(axis=1).sum(
                axis=1)  # sum over incoming interactions
    ...
    def sde_integrate(self, phases_vec, reduce_dim=False):
        """Updates all states by integrating state of all nodes, but sde version"""
        if self.type2 == 4:
            coupling2 = [self.coupling2, self.coupling2_sub]
            coupling3 = self.coupling3
        else:
        ...
        for i in range(1, t.size):
            ...
            _ = self.sd * dw_
            __ = dydt * dt
            next_step = y + dydt * dt + self.sd * dw_
            if self.type2 == 3 and self.type3 == 3:
                extincted = np.argwhere(next_step <= 0)
                to_append = extincted[~np.isin(extincted, extinct_sp)]
                extinct_sp = np.concatenate((extinct_sp, to_append))
                if len(extinct_sp) > 0:
                    for i_ in extinct_sp:
                        next_step[int(i_)] = 0
            ys[i] = next_step

        ans = ys.T

        if self.type2 == 3 and self.type3 == 3 and reduce_dim:
            ans = np.delete(ans, extinct_sp, axis=0)
            self.n_nodes -= len(extinct_sp)
            self.conn_mat2 = np.delete(self.conn_mat2, extinct_sp, axis=0)
            self.conn_mat2 = np.delete(self.conn_mat2, extinct_sp, axis=1)
            self.conn_mat3 = np.delete(self.conn_mat3, extinct_sp, axis=0)
            self.conn_mat3 = np.delete(self.conn_mat3, extinct_sp, axis=1)
            self.conn_mat3 = np.delete(self.conn_mat3, extinct_sp, axis=2)
            self.coupling2 = np.delete(self.coupling2, extinct_sp, axis=0)
            self.coupling2 = np.delete(self.coupling2, extinct_sp, axis=1)
            self.coupling3 = np.delete(self.coupling3, extinct_sp, axis=0)
            self.coupling3 = np.delete(self.coupling3, extinct_sp, axis=1)
            self.coupling3 = np.delete(self.coupling3, extinct_sp, axis=2)
            self.all_nodes = np.linspace(0, self.n_nodes - 1, self.n_nodes).astype(int)

        return ans  # transpose for consistency (act_mat:node vs time)

    def sde_integrate_test(self, phases_vec):
        """Only for LV dynamics"""
        coupling2 = self.coupling2  # normalize coupling by number of interactions
        coupling3 = self.coupling3  # normalize coupling by number of interactions
        ...
        test_dim = self.n_nodes
        ans = np.delete(ans, extinct_sp, axis=0)
        test_dim -= len(extinct_sp)

        return ans, test_dim  # transpose for consistency (act_mat:node vs time)
    ...
    def run(self, stretch=True, sim_stop=False, reduce_dim=False):
        """
        Returns
        -------
        act_mat: 2D ndarray
        ...
        if self.with_noise is False:
            ...
            result = self.sde_integrate(phases_vec, reduce_dim=reduce_dim)
    ...
    def test_run(self, stretch=True):
        """
        Now only for LV higher order dynamics.
        """
        phases_vec = self.init_phase

        result, test_result = self.sde_integrate_test(phases_vec)
    ...
    def make_demo_mat(self):
        # transfer matrix to connections
        demo_total2 = np.zeros(shape=(self.n_nodes, self.n_nodes - 1))
        demo_total3 = np.zeros(shape=(self.n_nodes, int((self.n_nodes - 1) * (self.n_nodes - 2) / 2)))
        if self.type2 == 5 and self.type3 == 5:
            demo_total4 = np.zeros(shape=(self.n_nodes, int((self.n_nodes - 1) * (self.n_nodes - 2) *
                                                            (self.n_nodes - 3) / 6)))
        if self.type2 == 6 and self.type3 == 6:
            demo_total3_sub = np.zeros(shape=(self.n_nodes, int((self.n_nodes - 1) * (self.n_nodes - 2) / 2)))
        ...
        for i in range(self.n_nodes):
            ...
            for j in others:
                ...
                if j == others[-1]:
                    ...
                    for k in more_others_lst[j]:
                        demo_lst3 = np.append(demo_lst3, self.coupling3[i][j][k])
                        if self.type2 == 6 and self.type3 == 6:
                            demo_lst3_sub = np.append(demo_lst3_sub, self.coupling3_sub[i][j][k])
                        if self.type2 == 5 and self.type3 == 5:
                            arr = more_others_lst[j]
                            others4 = arr[arr > k]
                            for l in others4:
                                demo_lst4 = np.append(demo_lst4, self.coupling4[i][j][k][l])
                else:
                    for k in more_others_lst[j - 1]:
                        demo_lst3 = np.append(demo_lst3, self.coupling3[i][j][k])
                        if self.type2 == 6 and self.type3 == 6:
            ...
            demo_total3[i, :] = demo_lst3
            if self.type2 == 5 and self.type3 == 5:
                demo_total4[i, :] = demo_lst4
            if self.type2 == 6 and self.type3 == 6:
                demo_total3_sub[i, :] = demo_lst3_sub

        demo_total = np.concatenate((demo_total2, demo_total3), axis=1)
        if self.type2 == 5 and self.type3 == 5:
            demo_total = np.concatenate((demo_total, demo_total4), axis=1)
        if self.type2 == 6 and self.type3 == 6:
            demo_total = np.concatenate((demo_total, demo_total3_sub), axis=1)

        return demo_total

    def make_result_mat(self, result):
        rows = len(result)
        cols = int(rows - 1 + (rows - 1) * (rows - 2) / 2)
        if self.type2 == 4 or self.type3 == 4 or self.type2 == 3 or self.type3 == 3:
            raise Exception("Design not complete. ")
        if self.type2 == 5 and self.type3 == 5:
            cols = int(cols + (rows - 1) * (rows - 2) * (rows - 3) / 6)
        if self.type2 == 6 and self.type3 == 6:
            cols = int(cols + (rows - 1) * (rows - 2) / 2)
        res = np.zeros(shape=(rows, cols))
        for i in range(rows):
            infer_arr = np.concatenate((result[i]["2"], result[i]["3"]))
            if self.type2 == 5 and self.type3 == 5:
                infer_arr = np.concatenate((infer_arr, result[i]["4"]))
            if self.type2 == 6 and self.type3 == 6:
                infer_arr = np.concatenate((infer_arr, result[i]["3_sub"]))
            res[i, :] = infer_arr

        return res

    def mse_and_r2_combined(self, results, tp_only=False, remove_tail=0):
        demo_total = self.make_demo_mat().reshape((-1))
        infer_total = self.make_result_mat(results).reshape((-1))
        if tp_only:
            test = np.logical_and(demo_total != 0, infer_total != 0)
            demo_total = demo_total[test]
            infer_total = infer_total[test]
        if remove_tail != 0:
            max_true = demo_total.max()
            test = np.logical_not(demo_total < max_true)
            demo_total = demo_total[test]
            infer_total = infer_total[test]
        assert len(demo_total) == len(infer_total), "mse_and_r2 boom"
        demo_avg = np.average(demo_total)
        rss = np.sum((demo_total - infer_total) ** 2)
        tss = np.sum((demo_total - demo_avg) ** 2)

        mse = 1 / len(demo_total) * rss
        R2 = 1 - rss / tss
        # print("The MSE of connectivity is: ", mse)
        # print("The coefficient of determination is: ", R2)

        corr_matrix = np.corrcoef(demo_total, infer_total)
    ...
    def prepare_diffs(self, results: np.array, test=False):
        """
        Testing.
        """
        ...
        for i in range(results.shape[0]):
            ...
            if self.type2 == 5:
                need_dim2 = int(((self.n_nodes - 1) * (self.n_nodes - 2) * (self.n_nodes - 3)) / 6)
                phi_diff41 = np.zeros(shape=(need_dim2, N - 1))
                phi_diff42 = np.zeros(shape=(need_dim2, N - 1))
                phi_diff43 = np.zeros(shape=(need_dim2, N - 1))
            elif self.type2 == 6 and self.type3 == 6:
                phi_diff3_sub2 = np.zeros(shape=(need_dim, N - 1))
            ...
            for j in others:
                if self.type2 == 1 or self.type2 == 4 or self.type2 == 5 or self.type2 == 6:
                    phi_diff_array = results[j] - self_phase
                elif self.type2 == 2:
                    phi_diff_array = 2 * (results[j] - self_phase)
                elif self.type2 == 0:
                    phi_diff_array = np.zeros(shape=results[j].size)
                if i > j:
                    phi_diff2[j] = phi_diff_array[:-1]
                elif i < j:
                    phi_diff2[j - 1] = phi_diff_array[:-1]
            ...
            if self.type3 != 4:
                ...
                for j in others:
                    ...
                    for k in more_others:
                        if self.type3 == 2:
                            phi_diff_array = results[j] + results[k] - 2 * self_phase
                        elif self.type3 == 1 or self.type2 == 5:
                            phi_diff_array = 2 * results[j] - results[k] - self_phase
                            phi_diff_array_sub = 2 * results[k] - results[j] - self_phase
                        elif self.type3 == 0:
                            phi_diff_array = np.zeros(shape=results[j].size)
                        elif self.type2 == 6 and self.type3 == 6:
                            phi_diff_array = 2 * results[j] - results[k] - self_phase
                            phi_diff_array_sub = 2 * results[k] - results[j] - self_phase
                            phi_diff_array_sub2 = results[j] + results[k] - 2 * self_phase
                        phi_diff3[counter] = phi_diff_array[:-1]
                        phi_diff3_sub[counter] = phi_diff_array_sub[:-1]
                        if self.type2 == 6 and self.type3 == 6:
                            phi_diff3_sub2[counter] = phi_diff_array_sub2[:-1]
                        counter += 1
                        if self.type2 == 5:
                            more_more_others = more_others[more_others > k]
                            for l in more_more_others:
                                phi_diff_array41 = results[j] + results[k] - results[l] - self_phase
                                phi_diff_array42 = results[j] + results[l] - results[k] - self_phase
                                phi_diff_array43 = results[k] + results[l] - results[j] - self_phase
                                phi_diff41[counter_] = phi_diff_array41[:-1]
                                phi_diff42[counter_] = phi_diff_array42[:-1]
                                phi_diff43[counter_] = phi_diff_array43[:-1]
                                counter_ += 1
            ...
            if self.type2 == 4:
                # cos the phase differences - 2, for Kuramoto-Sakaguchi
                twocos_lib = np.zeros(shape=(self.n_nodes - 1, N - 1))
                for i__ in range(phi_diff2.shape[0]):
                    cos_diff2 = np.cos(phi_diff2[i__])
                    twocos_lib[i__] = cos_diff2
            else:
                twocos_lib = None
            ...
            for i__ in range(need_dim):
                if self.type3 == 1 or self.type3 == 5:
                    sin_diff3 = np.sin(phi_diff3[i__]) + np.sin(phi_diff3_sub[i__])
                else:
                    sin_diff3 = np.sin(phi_diff3[i__])
                three_lib[i__] = sin_diff3

            if self.type2 == 5:
                four_lib = np.zeros(shape=(need_dim2, N - 1))
                for i__ in range(need_dim2):
                    sin_diff4 = np.sin(phi_diff41[i__]) + np.sin(phi_diff42[i__]) + np.sin(phi_diff43[i__])
                    four_lib[i__] = sin_diff4
            else:
                four_lib = None

            if self.type2 == 6:
                three_lib_sub = np.zeros(shape=(need_dim, N - 1))
                for i__ in range(need_dim):
                    sin_diff4 = np.sin(phi_diff3_sub2[i__])
                    three_lib_sub[i__] = sin_diff4
            else:
                three_lib_sub = None

            outputs = {"N": N, "self_phase": self_phase, "self_diff": self_diff, "two_lib": two_lib,
                       "three_lib": three_lib, "twocos_lib": twocos_lib, "four_lib": four_lib,
                       "three_lib_sub": three_lib_sub}
    ...
    def prepare_diffs_one_node(self, j, results: np.array):
        ...
        return {"N": N, "self_phase": self_phase, "self_diff": self_diff, "two_lib": two_lib,
                "three_lib": three_lib, "twocos_lib": None}
    ...
    def solve_ols(self, all_prepared):
        # Debug NaN values
        for i, node_data in enumerate(all_prepared):
            if np.any(np.isnan(node_data["self_diff"])):
                print(f"NaN detected in self_diff for node {i}")
            if np.any(np.isnan(node_data["two_lib"])):
                print(f"NaN detected in two_lib for node {i}")
            if np.any(np.isnan(node_data["three_lib"])):
                print(f"NaN detected in three_lib for node {i}")
        total_results = []
        ...
        for i in range(self.n_nodes):
            self_diff = all_prepared[i]["self_diff"]
            two_lib = all_prepared[i]["two_lib"]
            if node_data["twocos_lib"] is not None:
                twocos_lib = all_prepared[i]["twocos_lib"]
                two_lib = np.concatenate((two_lib, twocos_lib), axis=0)
            three_lib = all_prepared[i]["three_lib"]
            if self.type2 == 5:
                four_lib = all_prepared[i]["four_lib"]
            if self.type2 == 6 and self.type3 == 6:
                three_lib_sub = all_prepared[i]["three_lib_sub"]

            sin_diff_lib = np.concatenate((two_lib, three_lib), axis=0)
            if self.type2 == 5:
                sin_diff_lib = np.concatenate((sin_diff_lib, four_lib), axis=0)
            if self.type2 == 6 and self.type3 == 6:
                sin_diff_lib = np.concatenate((sin_diff_lib, three_lib_sub), axis=0)
            ...
            if np.isnan(transpose).any():
                print("NaN detected in transpose")
            if np.isnan(self_diff).any():
                print("NaN detected in self_diff")
            reg.fit(transpose, self_diff)
            two_conn_lst = reg.coef_[:self.n_nodes - 1]
            if node_data["twocos_lib"] is not None:
                twocos_conn_lst = reg.coef_[self.n_nodes - 1: 2 * self.n_nodes - 2]
                three_conn_lst = reg.coef_[2 * self.n_nodes - 2:]
                four_conn_lst = None
                three_conn_lst_sub = None
            elif self.type2 == 5:
                twocos_conn_lst = None
                need_end = int(self.n_nodes - 1 + int(((self.n_nodes - 1) * (self.n_nodes - 2)) / 2))
                three_conn_lst = reg.coef_[self.n_nodes - 1: need_end]
                four_conn_lst = reg.coef_[need_end:]
                three_conn_lst_sub = None
            elif self.type2 == 6 and self.type3 == 6:
                twocos_conn_lst = None
                need_end = int(self.n_nodes - 1 + int(((self.n_nodes - 1) * (self.n_nodes - 2)) / 2))
                three_conn_lst = reg.coef_[self.n_nodes - 1: need_end]
                three_conn_lst_sub = reg.coef_[need_end:]
                four_conn_lst = None
            else:
                twocos_conn_lst = None
            ...
            if self.type2 == 3 and self.type3 == 3:
                ...
                two_lst = np.concatenate((two_conn_lst, twocos_conn_lst))
                omega = reg.intercept_ % (2 * np.pi)
                error = self.dt / (len(self_diff) - len(two_conn_lst) - len(three_conn_lst) - 1) * \
                        np.sum((self_diff - omega
                                - np.dot(two_lst, two_lib)) ** 2)
            elif self.type2 == 5:
                omega = reg.intercept_ % (2 * np.pi)
                error = self.dt / (len(self_diff) - len(two_conn_lst) - len(three_conn_lst) - len(four_conn_lst) - 1) * \
                        np.sum((self_diff - omega - np.dot(two_conn_lst, two_lib)
                                - np.dot(three_conn_lst, three_lib) - np.dot(four_conn_lst, four_lib)) ** 2)
            elif self.type2 == 6 and self.type3 == 6:
                omega = reg.intercept_ % (2 * np.pi)
                error = self.dt / (len(self_diff) - len(two_conn_lst) -
                                   len(three_conn_lst) - len(three_conn_lst_sub) - 1) \
                        * np.sum((self_diff - omega - np.dot(two_conn_lst, two_lib) - np.dot(three_conn_lst, three_lib)
                                  - np.dot(three_conn_lst_sub, three_lib_sub)) ** 2)
            else:
                omega = reg.intercept_ % (2 * np.pi)
                error = self.dt / (len(self_diff) - len(two_conn_lst) - len(three_conn_lst) - 1) * \
                        np.sum((self_diff - omega
            ...
            results = {"2": two_conn_lst, "3": three_conn_lst, "natfreq": omega, "standard deviation": sd,
                       "2_cos": twocos_conn_lst, "4": four_conn_lst, "3_sub": three_conn_lst_sub}
            total_results.append(results)

        return total_results

    def solve_ols_one_node(self, all_prepared):
        self_diff = all_prepared["self_diff"]
        two_lib = all_prepared["two_lib"]
        if all_prepared["twocos_lib"] is not None:
            twocos_lib = all_prepared["twocos_lib"]
            two_lib = np.concatenate((two_lib, twocos_lib), axis=0)
        three_lib = all_prepared["three_lib"]
        ...
        two_conn_lst = reg.coef_[:self.n_nodes - 1]
        if all_prepared["twocos_lib"] is not None:
            twocos_conn_lst = reg.coef_[self.n_nodes: 2 * self.n_nodes - 1]
            three_conn_lst = reg.coef_[2 * self.n_nodes - 1:]
        else:
            twocos_conn_lst = None
            three_conn_lst = reg.coef_[self.n_nodes - 1:]
        ...
        return {"2": two_conn_lst, "3": three_conn_lst, "natfreq": omega, "standard deviation": sd,
                "2_cos": twocos_conn_lst}
    ...
    def solve_lasso(self, all_prepared):
        ...
        for i in range(self.n_nodes):
            ...
            results = {"2": two_conn_lst, "3": three_conn_lst, "natfreq": omega,
                       "2_cos": twocos_conn_lst, "4": four_conn_lst, "3_sub": three_conn_lst_sub}
            total_results.append(results)
    ...
    def solve_ada_lasso(self, all_prepared, mle_or_ols_results):
        ...
        for i in range(self.n_nodes):
            ...
            if self.type2 == 5:
                mle_4 = mle_or_ols_results[i]["4"]
            if self.type2 == 6 and self.type3 == 6:
                mle_3_sub = mle_or_ols_results[i]["3_sub"]
            ...
            if self.type2 == 5:
                mle_4_array = abs(mle_4)
                mle_array = np.append(mle_array, mle_4_array)
            if self.type2 == 6 and self.type3 == 6:
                mle_3_array_sub = abs(mle_3_sub)
                mle_array = np.append(mle_array, mle_3_array_sub)
            ...
            if self.type2 == 5:
                need_end = int(self.n_nodes - 1 + int(((self.n_nodes - 1) * (self.n_nodes - 2)) / 2))
                three_conn_lst = ada_reg.coef_[self.n_nodes - 1: need_end] * mle_3_array
                four_conn_lst = ada_reg.coef_[need_end:] * mle_4_array
                three_conn_lst_sub = None
            elif self.type2 == 6 and self.type3 == 6:
                need_end = int(self.n_nodes - 1 + int(((self.n_nodes - 1) * (self.n_nodes - 2)) / 2))
                three_conn_lst = ada_reg.coef_[self.n_nodes - 1: need_end] * mle_3_array
                three_conn_lst_sub = ada_reg.coef_[need_end:] * mle_3_array_sub
                four_conn_lst = None
            else:
            ...
            results = {"2": two_conn_lst, "3": three_conn_lst, "natfreq": omega, "alpha": alpha, "4": four_conn_lst,
                       "3_sub": three_conn_lst_sub}
            total_results.append(results)
    ...
    def solve_ada_group_lasso(self, all_prepared, mle_or_ols_results):
        ### TODO: Need to change when 3-body applied.
        if self.type2 != 4:
            raise Exception("Group LASSO is now only for Kuramoto-Sakaguchi like model")
        # Debug NaN values
        for i, node_data in enumerate(all_prepared):
            if np.any(np.isnan(node_data["self_diff"])):
                print(f"NaN detected in self_diff for node {i}")
            if np.any(np.isnan(node_data["two_lib"])):
                print(f"NaN detected in two_lib for node {i}")
            if np.any(np.isnan(node_data["twocos_lib"])):
                print(f"NaN detected in twocos_lib for node {i}")
            # if np.any(np.isnan(node_data["three_lib"])):
            #     print(f"NaN detected in three_lib for node {i}")
        ...
        for i in range(self.n_nodes):
            ...
            mle_2 = mle_or_ols_results[i]["2"]
            if mle_or_ols_results[i]["2_cos"] is not None:
                mle_2_cos = mle_or_ols_results[i]["2_cos"]
                mle_2 = np.concatenate((mle_2, mle_2_cos), axis=0)
            # mle_3 = mle_or_ols_results[i]["3"]

            # sin_diff_lib = np.concatenate((two_lib, three_lib), axis=0)
            sin_diff_lib = np.concatenate((two_lib, twocos_lib), axis=0)

            # WARNING: Need change when 3-body applied
            groups = [[i, i + self.n_nodes - 1] for i in range(self.n_nodes - 1)]

            weights = np.zeros(self.n_nodes - 1)
            for g, group in enumerate(groups):
                weights[g] = 1.0 / np.linalg.norm(mle_2[group])

            new_X = sin_diff_lib.copy()
            for g, group in enumerate(groups):
                new_X[group, :] /= weights[g]
            new_X = new_X.T

            # agl = GroupLasso(groups=groups, alpha=0.001, fit_intercept=False)
            # agl.fit(X_rescaled, self_diff)
            # beta_agl = agl.coef_

            gl = GroupLasso(groups=groups, alpha=0.001, fit_intercept=True)
            gl.fit(new_X, self_diff)
            result_lst = gl.coef_

            for g, group in enumerate(groups):
                result_lst[group] /= weights[g]

            two_conn_lst = result_lst[:self.n_nodes - 1]
            twocos_conn_lst = result_lst[self.n_nodes - 1:]

            omega = gl.intercept_ % (2 * np.pi)

            # results = {"2": two_conn_lst, "3": three_conn_lst, "natfreq": omega}
            results = {"2": two_conn_lst, "natfreq": omega, "2_cos": twocos_conn_lst}
            total_results.append(results)
    ...
    def demo_results(self, mle_or_ols_result=None, lasso_result=None, ada_result=None, agl_result=None):
        demo_coup2 = self.coupling2
        if self.type2 == 4:
            if agl_result is None:
                raise ValueError("agl_result cannot be None")
            demo_coup2_sub = self.coupling2_sub
        else:
            demo_coup3 = self.coupling3
            if self.type2 == 5:
                demo_coup4 = self.coupling4
            if self.type2 == 6 and self.type3 == 6:
                demo_coup3_sub = self.coupling3_sub
        ...
        for i in range(self.n_nodes):
            ...
            if self.type2 == 4:
                demo_lst2_sub = ["Real-2_sub"]
                for j in others:
                    demo_lst2_sub.append(demo_coup2_sub[j][i])
            else:
                demo_lst3 = ["Real-3"]
                if self.type2 == 5:
                    demo_lst4 = ["Real-4"]
                if self.type2 == 6 and self.type3 == 6:
                    demo_lst3_sub = ["Real-3_2"]
                for j in others:
                    if j == others[-1]:
                        break
                    elif j < i:
            ...
            table2 = [demo_lst2]
            if self.type2 == 4:
                table2_sub = [demo_lst2_sub]
            else:
                table3 = [demo_lst3]
                if self.type2 == 5:
                    table4 = [demo_lst4]
                if self.type2 == 6 and self.type3 == 6:
                    table3_sub = [demo_lst3_sub]
            ...
            if ada_result is not None:
                ...
                if self.type2 == 5:
                    four_conn_lst = ada_result[i]["4"]
                if self.type2 == 6 and self.type3 == 6:
                    three_conn_lst_sub = ada_result[i]["3_sub"]

                ada_results2 = two_conn_lst.tolist()
                ada_results3 = three_conn_lst.tolist()
                if self.type2 == 5:
                    ada_result4 = four_conn_lst.tolist()
                if self.type2 == 6 and self.type3 == 6:
                    ada_result3_sub = three_conn_lst_sub.tolist()

                ada_results2.insert(0, omega)
                ada_results2.insert(0, "Estimates-2 - Ada. LASSO")
                ada_results3.insert(0, "Estimates-3 - Ada. LASSO")
                if self.type2 == 5:
                    ada_result4.insert(0, "Estimate-4 - Ada. LASSO")
                if self.type2 == 6 and self.type3 == 6:
                    ada_result3_sub.insert(0, "Estimate-3_2 - Ada. LASSO")

                table2.append(ada_results2)
                table3.append(ada_results3)
                if self.type2 == 5:
                    table4.append(ada_result4)
                if self.type2 == 6 and self.type3 == 6:
                    table3_sub.append(ada_result3_sub)
            ...
            if mle_or_ols_result is not None:
                ...
                if self.type2 != 4:
                    ...
                    if self.type2 == 5:
                        mle_estimate4 = ["Estimates-4 - OLS"] + mle_or_ols_result[i]["4"].tolist()
                        table4.append(mle_estimate4)
                    if self.type2 == 6 and self.type3 == 6:
                        mle_estimate3_sub = ["Estimates-3_2 - OLS"] + mle_or_ols_result[i]["3_sub"].tolist()
                        table3_sub.append(mle_estimate3_sub)
                else:
                    mle_estimates2_cos = ["Estimates-2_cos - OLS"] + mle_or_ols_result[i]["2_cos"].tolist()
                    table2_sub.append(mle_estimates2_cos)

            if agl_result is not None:
                two_conn_lst = agl_result[i]["2"]
                twosub_conn_lst = agl_result[i]["2_cos"]
                # three_conn_lst = agl_result[i]["3"]
                omega = agl_result[i]["natfreq"]

                agl_results2 = two_conn_lst.tolist()
                agl_results2_cos = twosub_conn_lst.tolist()

                agl_results2.insert(0, omega)
                agl_results2.insert(0, "Estimates-2 - AG LASSO")
                agl_results2_cos.insert(0, "Estimates-2_cos - AG LASSO")

                table2.append(agl_results2)
                table2_sub.append(agl_results2_cos)
                # table3.append(ada_results3)

            col_names2 = ["Form: k_ij", "natural frequencies_" + str(i + 1)]
            counter_2 = 0
            for index in others:
                name = "k_" + str(i + 1) + str(index + 1)
            ...
            if self.type2 != 4:
                col_names3 = ["Form: k_ijl"]
                if self.type2 == 5:
                    col_names4 = ["Form: k_ijlm"]
                if self.type2 == 6 and self.type3 == 6:
                    col_names3_sub = ["Form: k_ijl"]
                for index in others:
                    if index == others[-1]:
                        break
                    elif index < i:
                        for inde2 in more_others_lst[index]:
                            name = "k_" + str(i + 1) + str(index + 1) + str(inde2 + 1)
                            col_names3.append(name)
                            if self.type2 == 5:
                                arr = more_others_lst[index]
                                others4 = arr[arr > inde2]
                                for l in others4:
                                    name = "k_" + str(i + 1) + str(index + 1) + str(inde2 + 1) + str(l + 1)
                                    col_names4.append(name)
                            if self.type2 == 6 and self.type3 == 6:
                                col_names3_sub.append(name)
                ...
                if self.type2 == 5:
                    print("/////////////////////////////////////")
                    print(tabulate(table4, headers=col_names4))
                    total_table_4.append(table4)
                if self.type2 == 6 and self.type3 == 6:
                    print("/////////////////////////////////////")
                    print(tabulate(table3_sub, headers=col_names3_sub))
                    total_table_3_sub.append(table3_sub)
                print("====================================================================")
                total_table_3.append(table3)

            else:
                col_names2_sub = ["Form: k_ji for cos"]
                counter_2_sub = 0
                for index in others:
                    name = "k_" + str(index + 1) + str(i + 1)
                    col_names2_sub.append(name)
                    counter_2_sub += 1
                print(tabulate(table2_sub, headers=col_names2_sub))
                print("/////////////////////////////////////")
                total_table_2_sub.append(table2_sub)

        if self.type2 == 5:
            third_result = total_table_4
        elif self.type2 != 6 and self.type3 != 6:
            third_result = total_table_3_sub
        else:
            third_result = None

        if self.type2 == 4:
            return total_table_2, total_table_2_sub, third_result
        return total_table_2, total_table_3, third_result

    def demo_solve(self, mle=False, lasso=False, ada=False, print_result=False, op_illu=False):
        act_mat = self.run()
        prepare_all = self.prepare_diffs(act_mat)

        if op_illu:
            print(avg_phase_coherence(act_mat))
        ...
        if print_result:
            _, _, _ = self.demo_results(mle_or_ols_result=mle_or_ols_results, lasso_result=lasso_results,
    ...
    def demo_solve_one_node(self, mle=False, lasso=False, ada=False):
        ...
        for i in range(self.n_nodes):
            ...
            if ada and not mle:
                raise Exception("Need 'mle' to be true if 'ada' is true. ")
            if mle:
                mle_or_ols_result = self.solve_ols_one_node(all_prepared=prepare_all)
    ...
    def fdr_basic(self, inf_result_real, node_num):
        ...
        # conn = None here for convenience to avoid assertion error
        model_non = GeneralInteraction(coupling2=0, coupling3=0, dt=self.dt, T=self.T, natfreqs=nats_inf,
                                       ...
                                       starts_from=self.starts_from, inf_last=self.inf_last, conn=None,
                                       for_fdr_empty=True, type2=self.type2, type3=self.type3, conn4=0,
                                       conn3_sub=0,
                                       pre_conn4=np.zeros(
                                           shape=(self.n_nodes, self.n_nodes, self.n_nodes, self.n_nodes)),
                                       pre_coup4=np.zeros(
                                           shape=(self.n_nodes, self.n_nodes, self.n_nodes, self.n_nodes)),
                                       pre_conn3_sub=np.zeros(
                                           shape=(self.n_nodes, self.n_nodes, self.n_nodes)),
                                       pre_coup3_sub=np.zeros(
                                           shape=(self.n_nodes, self.n_nodes, self.n_nodes)))
        if self.n_nodes >= 50:
            inf_result_0, _, _ = model_non.demo_solve_one_node(mle=True, ada=False, lasso=False)
        else:
            ts_0 = model_non.run()
        ...
        if self.type2 == 5:
            css4_arr = np.array([])
        if self.type2 == 6 and self.type3 == 6:
            css3_arr_sub = np.array([])
        for i in range(self.n_nodes):
            css2_arr = np.concatenate((css2_arr, inf_result_0[i]["2"]), axis=0)
            css3_arr = np.concatenate((css3_arr, inf_result_0[i]["3"]), axis=0)
            if self.type2 == 5:
                css4_arr = np.concatenate((css4_arr, inf_result_0[i]["4"]), axis=0)
            if self.type2 == 6 and self.type3 == 6:
                css3_arr_sub = np.concatenate((css3_arr_sub, inf_result_0[i]["3_sub"]), axis=0)
        ...
        if self.type2 == 5:
            sp4_mean = np.mean(css4_arr)
            sp4_var = np.var(css4_arr, ddof=1)
            sp4_sd = np.sqrt(sp4_var)
            add_ = [sp4_mean, sp4_var, sp4_sd]
        elif self.type2 == 6 and self.type3 == 6:
            sp3_mean_sub = np.mean(css3_arr_sub)
            sp3_var_sub = np.var(css3_arr_sub, ddof=1)
            sp3_sd_sub = np.sqrt(sp3_var_sub)
            add_ = [sp3_mean_sub, sp3_var_sub, sp3_sd_sub]
        else:
            add_ = None

        return [sp2_mean, sp2_var, sp2_sd, sp3_mean, sp3_var, sp3_sd, add_]
    ...
    def fdr_control_for_mle(self, inf_result_real, sig_level=0.05):
        ...
        sp3_var = resultss[4]
        if self.type2 == 5:
            assert resultss[6] is not None, "fdr_control_for_mle Boomba! "
            sp4_mean = resultss[6][0]
            sp4_var = resultss[6][1]
        if self.type2 == 6 and self.type3 == 6:
            assert resultss[6] is not None, "fdr_control_for_mle Boomba! "
            sp3_mean_sub = resultss[6][0]
            sp3_var_sub = resultss[6][1]
        ...
        if self.type2 == 5:
            p_value4 = np.zeros(shape=(node_num, int((node_num - 1) * (node_num - 2) * (node_num - 3) / 6)))
        if self.type2 == 6 and self.type3 == 6:
            p_value3_sub = np.zeros(shape=(node_num, int((node_num - 1) * (node_num - 2) / 2)))
        for i in range(self.n_nodes):
            # 2 adds
            ...
            if self.type2 == 5:
                result__ = inf_result_real[self.all_nodes[i]]["4"].copy()
                p_s = double_tailed_p_value(result__, sp4_mean, sp4_var)
                p_value4[i] = p_s

            if self.type2 == 6 and self.type3 == 6:
                result___ = inf_result_real[self.all_nodes[i]]["3_sub"].copy()
                p_s = double_tailed_p_value(result___, sp3_mean_sub, sp3_var_sub)
                p_value3_sub[i] = p_s

        p2_adj = np.reshape(p_value2, p_value2.size)
        p3_adj = np.reshape(p_value3, p_value3.size)
        if self.type2 == 5:
            p4_adj = np.reshape(p_value4, p_value4.size)
        if self.type2 == 6 and self.type3 == 6:
            p3_adj_sub = np.reshape(p_value3_sub, p_value3_sub.size)
        ...
        if self.type2 == 5:
            rejected4, test4, _, _ = multipletests(p4_adj, alpha=sig_level, method='fdr_bh')
            back4 = np.reshape(test4, p_value4.shape)
            p_value4 = back4

        if self.type2 == 6 and self.type3 == 6:
            rejected3_sub, test3_sub, _, _ = multipletests(p3_adj_sub, alpha=sig_level, method='fdr_bh')
            back3_sub = np.reshape(test3_sub, p_value3_sub.shape)
            p_value3_sub = back3_sub
        ...
        for i in range(len(self.all_nodes)):
            ...
            if self.type2 == 5:
                conn4_ = np.zeros(shape=int((node_num - 1) * (node_num - 2) * (node_num - 3) / 6))
                for j in range(conn4_.shape[0]):
                    if p_value4[i][j] <= sig_level:
                        conn4_[j] = 1
            else:
                conn4_ = None
            if self.type2 == 6 and self.type3 == 6:
                conn3__sub = np.zeros(shape=int((node_num - 1) * (node_num - 2) / 2))
                for j in range(conn3__sub.shape[0]):
                    if p_value3_sub[i][j] <= sig_level:
                        conn3__sub[j] = 1
            else:
                conn3__sub = None

            conn_all.append({"2": conn2_, "3": conn3_, "4": conn4_, "3_sub": conn3__sub})
        return conn_all, resultss

    def mle_with_fdr(self, mle_or_ols_results, fdr_results, compact=False):
        if compact:
            # TODO: Incorporate with 4-body and general 3-body
        ...
        for i in range(self.n_nodes):
            ...
            if self.type2 == 5:
                results4 = mle_or_ols_results[i]["4"] * fdr_results[i]["4"]
            else:
                results4 = None
            if self.type2 == 6 and self.type3 == 6:
                results3_sub = mle_or_ols_results[i]["3_sub"] * fdr_results[i]["3_sub"]
            else:
                results3_sub = None
            ...
            if compact:
                ...
                coup.append({"natfreq": mle_or_ols_results[i]["natfreq"], "2": results2, "3": results3, "4": results4,
                             "3_sub": results3_sub,
    ...
    def conn_criteria_base(self, mle_or_ols_results=None, lasso_results=None, ada_results=None, sig_level=0.05,
                           mle_ori=False,
                           mle_threshold=0, conn_out=False, weak_strong=0, threshold_low=0):
        demo_coup2 = self.coupling2
        demo_coup3 = self.coupling3
        ...
        if mle_or_ols_results is not None:
            ...
            mle_output = {"A": 0, "B": 0, "TP": 0, "TN": 0, "FP": 0, "FN": 0,
                          ...
                          "TP_3_sub": 0, "TN_3_sub": 0, "FP_3_sub": 0, "FN_3_sub": 0,
                          "TP_4": 0, "TN_4": 0, "FP_4": 0, "FN_4": 0,
            ...
            if mle_ori:
                ...
                sp3_sd = paramss[5]
                if self.type2 == 5:
                    sp4_mean = paramss[6][0]
                    sp4_sd = paramss[6][2]
                if self.type2 == 6 and self.type3 == 6:
                    sp3_mean_sub = paramss[6][0]
                    sp3_sd_sub = paramss[6][2]

        lasso_output = None
        if lasso_results is not None:
            assert len(lasso_results) == self.n_nodes, "lasso_results BOOM conn_criteria"
            lasso_output = {"A": 0, "B": 0, "TP": 0, "TN": 0, "FP": 0, "FN": 0,
                            "TP_2": 0, "TN_2": 0, "FP_2": 0, "FN_2": 0,
                            "TP_3": 0, "TN_3": 0, "FP_3": 0, "FN_3": 0,
                            "TP_3_sub": 0, "TN_3_sub": 0, "FP_3_sub": 0, "FN_3_sub": 0,
                            "TP_4": 0, "TN_4": 0, "FP_4": 0, "FN_4": 0,
        ...
        if ada_results is not None:
            ...
            ada_output = {"A": 0, "B": 0, "TP": 0, "TN": 0, "FP": 0, "FN": 0,
                          "TP_2": 0, "TN_2": 0, "FP_2": 0, "FN_2": 0,
                          "TP_3": 0, "TN_3": 0, "FP_3": 0, "FN_3": 0,
                          "TP_3_sub": 0, "TN_3_sub": 0, "FP_3_sub": 0, "FN_3_sub": 0,
                          "TP_4": 0, "TN_4": 0, "FP_4": 0, "FN_4": 0,
        ...
        for i in range(self.n_nodes):
            ...
            for j in others:
                ...
                if j == others[-1]:
                    ...
                    for k in more_others_lst[j]:
                        demo_lst3.append(demo_coup3[i][j][k])
                        if self.type2 == 6 and self.type3 == 6:
                            demo_lst3_sub.append(self.coupling3_sub[i][j][k])
                        if self.type2 == 5:
                            arr = more_others_lst[j]
                            others4 = arr[arr > k]
                            for l in others4:
                                demo_lst4.append(self.coupling4[i][j][k][l])
                else:
                    for k in more_others_lst[j - 1]:
                        demo_lst3.append(demo_coup3[i][j][k])
                        if self.type2 == 6 and self.type3 == 6:
            ...
            two_real[abs(two_real) > threshold_low] = 1
            two_real[abs(two_real) <= threshold_low] = 0
            three_real = np.array(demo_lst3)
            three_real[abs(three_real) > threshold_low] = 1
            three_real[abs(three_real) <= threshold_low] = 0
            if self.type2 == 5:
                four_real = np.array(demo_lst4)
                four_real[abs(four_real) > threshold_low] = 1
                four_real[abs(four_real) <= threshold_low] = 0
            if self.type2 == 6 and self.type3 == 6:
                three_real_sub = np.array(demo_lst3_sub)
                three_real_sub[abs(three_real_sub) > threshold_low] = 1
                three_real_sub[abs(three_real_sub) <= threshold_low] = 0
            ...
            if lasso_results is not None:
                ...
                if self.type2 == 5:
                    four_conn_lst_lasso = lasso_results[i]["4"].copy()
                if self.type2 == 6 and self.type3 == 6:
                    three_conn_lst_lasso_sub = lasso_results[i]["3_sub"].copy()

                # two_conn_lst_lasso[two_conn_lst_lasso != 0] = 1
                # three_conn_lst_lasso[three_conn_lst_lasso != 0] = 1
                two_conn_lst_lasso[abs(two_conn_lst_lasso) > threshold_low] = 1
                two_conn_lst_lasso[abs(two_conn_lst_lasso) <= threshold_low] = 0
                three_conn_lst_lasso[abs(three_conn_lst_lasso) > threshold_low] = 1
                three_conn_lst_lasso[abs(three_conn_lst_lasso) <= threshold_low] = 0
                ...
                if self.type2 == 5:
                    TP_values_lasso_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_lasso != 0))
                    TN_values_lasso_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_lasso == 0))
                    FP_values_lasso_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_lasso != 0))
                    FN_values_lasso_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_lasso == 0))

                if self.type2 == 6 and self.type3 == 6:
                    TP_values_lasso_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_lasso_sub != 0))
                    TN_values_lasso_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_lasso_sub == 0))
                    FP_values_lasso_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_lasso_sub != 0))
                    FN_values_lasso_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_lasso_sub == 0))
                ...
                if self.type2 == 5:
                    TP_values_lasso += TP_values_lasso_4
                    TN_values_lasso += TN_values_lasso_4
                    FP_values_lasso += FP_values_lasso_4
                    FN_values_lasso += FN_values_lasso_4
                if self.type2 == 6 and self.type3 ==6:
                    TP_values_lasso += TP_values_lasso_3_sub
                    TN_values_lasso += TN_values_lasso_3_sub
                    FP_values_lasso += FP_values_lasso_3_sub
                    FN_values_lasso += FN_values_lasso_3_sub
                ...
                if self.type2 == 5:
                    lasso_output["TP_4"] += TP_values_lasso_4
                    lasso_output["TN_4"] += TN_values_lasso_4
                    lasso_output["FP_4"] += FP_values_lasso_4
                    lasso_output["FN_4"] += FN_values_lasso_4
                if self.type2 == 6 and self.type3 == 6:
                    lasso_output["TP_3_sub"] += TP_values_lasso_3_sub
                    lasso_output["TN_3_sub"] += TN_values_lasso_3_sub
                    lasso_output["FP_3_sub"] += FP_values_lasso_3_sub
                    lasso_output["FN_3_sub"] += FN_values_lasso_3_sub
            ...
            if mle_or_ols_results is not None:
                ...
                two_conn_lst_mle[abs(two_conn_lst_mle) > threshold_low] = 1
                two_conn_lst_mle[abs(two_conn_lst_mle) <= threshold_low] = 0
                three_conn_lst_mle[abs(three_conn_lst_mle) > threshold_low] = 1
                three_conn_lst_mle[abs(three_conn_lst_mle) <= threshold_low] = 0

                if self.type2 == 5:
                    four_conn_lst_mle = mle_conn_lst_total[i]["4"].copy()
                if self.type2 == 6 and self.type3 == 6:
                    three_conn_lst_mle_sub = mle_conn_lst_total[i]["3_sub"].copy()

                ee = np.count_nonzero(two_conn_lst_mle)
                ff = np.count_nonzero(three_conn_lst_mle)

                TP_values_mle_2 = np.sum(np.logical_and(two_real == 1, two_conn_lst_mle != 0))
                TN_values_mle_2 = np.sum(np.logical_and(two_real == 0, two_conn_lst_mle == 0))
                FP_values_mle_2 = np.sum(np.logical_and(two_real == 0, two_conn_lst_mle != 0))
                FN_values_mle_2 = np.sum(np.logical_and(two_real == 1, two_conn_lst_mle == 0))

                TP_values_mle_3 = np.sum(np.logical_and(three_real == 1, three_conn_lst_mle != 0))
                TN_values_mle_3 = np.sum(np.logical_and(three_real == 0, three_conn_lst_mle == 0))
                FP_values_mle_3 = np.sum(np.logical_and(three_real == 0, three_conn_lst_mle != 0))
                FN_values_mle_3 = np.sum(np.logical_and(three_real == 1, three_conn_lst_mle == 0))

                if self.type2 == 5:
                    TP_values_mle_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_mle != 0))
                    TN_values_mle_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_mle == 0))
                    FP_values_mle_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_mle != 0))
                    FN_values_mle_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_mle == 0))

                if self.type2 == 6 and self.type3 == 6:
                    TP_values_mle_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_mle_sub != 0))
                    TN_values_mle_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_mle_sub == 0))
                    FP_values_mle_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_mle_sub != 0))
                    FN_values_mle_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_mle_sub == 0))
                ...
                if weak_strong > 0:
                    ...
                    FP_values_mle_2_weak = np.sum(np.logical_and(two_real_ws == 0, two_conn_lst_mle != 0))
                    ...
                    FP_values_mle_2_strong = np.sum(np.logical_and(two_real == 0, two_conn_lst_mle != 0))
                    ...
                    FP_values_mle_3_weak = np.sum(np.logical_and(three_real_ws == 0, three_conn_lst_mle != 0))
                    ...
                    FP_values_mle_3_strong = np.sum(np.logical_and(three_real == 0, three_conn_lst_mle != 0))
                ...
                if self.type2 == 5:
                    TP_values_mle += TP_values_mle_4
                    TN_values_mle += TN_values_mle_4
                    FP_values_mle += FP_values_mle_4
                    FN_values_mle += FN_values_mle_4
                if self.type2 == 6 and self.type3 ==6:
                    TP_values_mle += TP_values_mle_3_sub
                    TN_values_mle += TN_values_mle_3_sub
                    FP_values_mle += FP_values_mle_3_sub
                    FN_values_mle += FN_values_mle_3_sub
                ...
                if self.type2 != 5 and self.type2 != 6 and self.type2 != 6:
                ...
                if self.type2 == 5:
                    mle_output["TP_4"] += TP_values_mle_4
                    mle_output["TN_4"] += TN_values_mle_4
                    mle_output["FP_4"] += FP_values_mle_4
                    mle_output["FN_4"] += FN_values_mle_4
                if self.type2 == 6 and self.type3 == 6:
                    mle_output["TP_3_sub"] += TP_values_mle_3_sub
                    mle_output["TN_3_sub"] += TN_values_mle_3_sub
                    mle_output["FP_3_sub"] += FP_values_mle_3_sub
                    mle_output["FN_3_sub"] += FN_values_mle_3_sub
                ...
                if mle_ori:
                    ...
                    if weak_strong > 0:
                        ...
                        FP_values_mle_ori_2_weak = np.sum(np.logical_and(two_real_ws == 0, two_conn_lst_mle != 0))
                        ...
                        FP_values_mle_ori_2_strong = np.sum(np.logical_and(two_real == 0, two_conn_lst_mle != 0))
                        ...
                        FP_values_mle_ori_3_weak = np.sum(np.logical_and(three_real_ws == 0, three_conn_lst_mle != 0))
                        ...
                        FP_values_mle_ori_3_strong = np.sum(np.logical_and(three_real == 0, three_conn_lst_mle != 0))
            ...
            if ada_results is not None:
                ...
                two_conn_lst_ada[abs(two_conn_lst_ada) > threshold_low] = 1
                two_conn_lst_ada[abs(two_conn_lst_ada) <= threshold_low] = 0
                three_conn_lst_ada[abs(three_conn_lst_ada) > threshold_low] = 1
                three_conn_lst_ada[abs(three_conn_lst_ada) <= threshold_low] = 0

                if self.type2 == 5:
                    four_conn_lst_ada = ada_results[i]["4"].copy()
                if self.type2 == 6 and self.type3 == 6:
                    three_conn_lst_ada_sub = ada_results[i]["3_sub"].copy()

                aa = np.count_nonzero(two_conn_lst_ada)
                bb = np.count_nonzero(three_conn_lst_ada)

                if self.type2 == 5:
                    four_conn_lst_ada[abs(four_conn_lst_ada) > threshold_low] = 1
                    four_conn_lst_ada[abs(four_conn_lst_ada) <= threshold_low] = 0
                if self.type2 == 6 and self.type3 == 6:
                    three_conn_lst_ada_sub[abs(three_conn_lst_ada_sub) > threshold_low] = 1
                    three_conn_lst_ada_sub[abs(three_conn_lst_ada_sub) <= threshold_low] = 0
                ...
                if self.type2 == 5:
                    TP_values_ada_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_ada == 1))
                    TN_values_ada_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_ada == 0))
                    FP_values_ada_4 = np.sum(np.logical_and(four_real == 0, four_conn_lst_ada == 1))
                    FN_values_ada_4 = np.sum(np.logical_and(four_real == 1, four_conn_lst_ada == 0))

                if self.type2 == 6 and self.type3 == 6:
                    TP_values_ada_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_ada_sub == 1))
                    TN_values_ada_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_ada_sub == 0))
                    FP_values_ada_3_sub = np.sum(np.logical_and(three_real_sub == 0, three_conn_lst_ada_sub == 1))
                    FN_values_ada_3_sub = np.sum(np.logical_and(three_real_sub == 1, three_conn_lst_ada_sub == 0))
                ...
                if self.type2 == 5:
                    TP_values_ada += TP_values_ada_4
                    TN_values_ada += TN_values_ada_4
                    FP_values_ada += FP_values_ada_4
                    FN_values_ada += FN_values_ada_4
                if self.type2 == 6 and self.type3 == 6:
                    TP_values_ada += TP_values_ada_3_sub
                    TN_values_ada += TN_values_ada_3_sub
                    FP_values_ada += FP_values_ada_3_sub
                    FN_values_ada += FN_values_ada_3_sub
                ...
                if self.type2 == 5:
                    ada_output["TP_4"] += TP_values_ada_4
                    ada_output["TN_4"] += TN_values_ada_4
                    ada_output["FP_4"] += FP_values_ada_4
                    ada_output["FN_4"] += FN_values_ada_4
                if self.type2 == 6 and self.type3 == 6:
                    ada_output["TP_3_sub"] += TP_values_ada_3_sub
                    ada_output["TN_3_sub"] += TN_values_ada_3_sub
                    ada_output["FP_3_sub"] += FP_values_ada_3_sub
                    ada_output["FN_3_sub"] += FN_values_ada_3_sub
